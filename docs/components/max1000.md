...
The board is equipped with an integrated Arrow USB-Programmer that enables the FPGA to be programmed directly from a PC and debugged using the free of charge Intel Quartus Prime Lite software. This makes Arrow MAX1000 an easy to use plug and play solution. The Arrow USB-Programmer can also be integrated into the user’s own circuit if required.

<div class="fluidMedia"><iframe id="ytplayer" type="text/html" width="100%" src="https://www.youtube.com/embed/LB33QsB-Bj0?autoplay=0&origin=http://vhdplus.com" allowFullScreen></iframe></div>
