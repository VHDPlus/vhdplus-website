...
export const IDEVersion = '0.9.5.6';
...
The VHDP Core is equipped with a MAX 10 FPGA. Select the device family depending on your development board.
...
Manual:
1. Start VHDP IDE and create a new project (top left), thiss will automatically create a .vhdp file.
...
Before clicking on OK, check to ensure if Quartus' path is correct. On Windows this path is usually `C:\intelFPGA_lite\18.1\quartus"`
...
5. When compiling is happening, allow several minutes for it to finish, as the compiler attempts to find the most efficient way to connect the logic elements. Once compiling is completed you can program the FPGA. To do that you must ensure that the correct drivers are installed and to click on Program (blue arrow). If you encounter any problems, use the Quartus programming window, next to the Program button.
