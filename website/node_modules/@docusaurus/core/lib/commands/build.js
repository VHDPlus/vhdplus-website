...
async function build(siteDir, cliOptions = {}) {
    ...
    const chunkManifestPath = path_1.default.join(generatedFilesDir, 'chunk-map.json');
    await Promise.all([clientManifestPath, chunkManifestPath].map(async (manifestPath) => {
        const manifestExist = await fs_extra_1.default.pathExists(manifestPath);
        if (manifestExist) {
            await fs_extra_1.default.unlink(manifestPath);
        }
    }));
